...
{
    public sealed class OldEvaluation
    {
        public const ulong QUEEN_SIDE_MASK = 0x0F0F0F0F0F0F0F0Ful;
        public const ulong KING_SIDE_MASK = 0xF0F0F0F0F0F0F0F0ul;

        static OldEvaluation()
        {
            LoadWeights();
        }

        public OldEvaluation(bool adjMat = true)
        {
            adjustMaterial = adjMat;
        }
        ...
        public short Compute(Board board)
        {
            ...
            if (gamePhase == GamePhase.EndGameMopUp)
            {
                short d = (short)(Index.Distance(kingIndex[0], kingIndex[1]) * EndGameMopUpKingDistanceWeight);
        ...
        public short AdjustMaterial(int material, int adjustWt)
        {
            return adjustMaterial ? (short)((material * adjustWt) / 10) : (short)material;
        }
        ...
        public static void GetGamePhase(Board board, out GamePhase gamePhase, out int opWt, out int egWt)
        {
            ...
            else if (board.Material(Color.White) + board.Material(Color.Black) <= EndGamePhaseMaterial)
            {
                ...
                gamePhase = totalPawns == 0 ? GamePhase.EndGameMopUp : GamePhase.EndGame;
                opWt = 0;
                egWt = 128;
            }
            else
        ...
        private bool adjustMaterial = true;
        ...
        public static short EndGameMopUpKingDistanceWeight =>
            weights.Weights[ChessWeights.ENDGAME_KING_DIST_PENALTY_WEIGHT_OFFSET];
        ...
        private static ReadOnlyMemory<short> opKingClosest = new(Array.Empty<short>());
        private static ReadOnlyMemory<short> egKingClosest = new(Array.Empty<short>()); 
