...
namespace Pedantic.UnitTests
{
    ...
    public class EvalFeaturesTests
    {
        ...
        public void ComputeTest(string fen)
        {
            ...
            HceWeights weights = Evaluation2.Weights;

            Span<Evaluation2.EvalInfo> evalInfo = stackalloc Evaluation2.EvalInfo[2];
            Evaluation2.InitializeEvalInfo(bd, evalInfo);

            Evaluation2 eval = new(cache, useMopup: false);

            // material + pst
            Score score = Evaluation2.EvalMaterialAndPst(bd, evalInfo, Color.White);
            score -= Evaluation2.EvalMaterialAndPst(bd, evalInfo, Color.Black);
            short evalScore = Evaluation2.StmScore(bd.SideToMove, score.NormalizeScore(bd.Phase));
            ...
            evalScore = Evaluation2.StmScore(bd.SideToMove, score.NormalizeScore(bd.Phase));
            featureScore = features.Compute(weights, HceWeights.ISOLATED_PAWN, HceWeights.KING_OUTSIDE_PP_SQUARE);
            Assert.AreEqual(evalScore, featureScore);

            // mobility
            score = Evaluation2.EvalMobility(bd, evalInfo, Color.White);
            score -= Evaluation2.EvalMobility(bd, evalInfo, Color.Black);
            evalScore = Evaluation2.StmScore(bd.SideToMove, score.NormalizeScore(bd.Phase));
            featureScore = features.Compute(weights, HceWeights.PIECE_MOBILITY, HceWeights.CENTER_CONTROL);
            Assert.AreEqual(evalScore, featureScore);
            
            // king safety
            score = Evaluation2.EvalKingSafety(bd, evalInfo, Color.White);
            score -= Evaluation2.EvalKingSafety(bd, evalInfo, Color.Black);
            evalScore = Evaluation2.StmScore(bd.SideToMove, score.NormalizeScore(bd.Phase));
            featureScore = features.Compute(weights, HceWeights.KING_ATTACK, HceWeights.ISOLATED_PAWN);
            Assert.AreEqual(evalScore, featureScore);

            // pieces
            score = Evaluation2.EvalPieces(bd, evalInfo, Color.White);
            score -= Evaluation2.EvalPieces(bd, evalInfo, Color.Black);
            evalScore = Evaluation2.StmScore(bd.SideToMove, score.NormalizeScore(bd.Phase));
            featureScore = features.Compute(weights, HceWeights.KNIGHT_OUTPOST, HceWeights.PAWN_PUSH_THREAT);
            Assert.AreEqual(evalScore, featureScore);

            // passed pawns
            score = Evaluation2.EvalPassedPawns(bd, evalInfo, Color.White);
            score -= Evaluation2.EvalPassedPawns(bd, evalInfo, Color.Black);
            evalScore = Evaluation2.StmScore(bd.SideToMove, score.NormalizeScore(bd.Phase));
            featureScore = features.Compute(weights, HceWeights.KING_OUTSIDE_PP_SQUARE, HceWeights.KNIGHT_OUTPOST);
            Assert.AreEqual(evalScore, featureScore);

            // threats
            score = Evaluation2.EvalThreats(bd, evalInfo, Color.White);
            score -= Evaluation2.EvalThreats(bd, evalInfo, Color.Black);
            evalScore = Evaluation2.StmScore(bd.SideToMove, score.NormalizeScore(bd.Phase));
            featureScore = features.Compute(weights, HceWeights.PAWN_PUSH_THREAT, HceWeights.MAX_WEIGHTS);
            Assert.AreEqual(evalScore, featureScore);

            // miscellaneous
            score = Evaluation2.EvalMisc(bd, evalInfo, Color.White);
            score -= Evaluation2.EvalMisc(bd, evalInfo, Color.Black);
            evalScore = Evaluation2.StmScore(bd.SideToMove, score.NormalizeScore(bd.Phase));
        ...
        [DataRow("r1bqr1k1/pp1p1ppp/2p5/8/3N1Q2/P2BB3/1PP2PPP/R3K2n b Q - 1 12")]
        [DataRow("r1bq2k1/p4r1p/1pp2pp1/3p4/1P1B3Q/P2B1N2/2P3PP/4R1K1 b - - 2 19")]
        public void FindDifference(string fen)
        {
            Board bd = new(fen);
            EvalFeatures features = new(bd);
            HceWeights weights = Evaluation2.Weights;
            short eval1 = features.Compute(weights);

            Evaluation oldEval = new Evaluation(false, false, false);
            short eval2 = oldEval.Compute(bd);
            EvalParts parts = oldEval.EvalParts;

            Console.WriteLine($"New: {eval1}, Old: {eval2}, Phase: {features.Phase}");
            PrintSparseArray(features, weights);

            // material + pst
            int featureScore = features.Compute(weights, 0, HceWeights.PIECE_MOBILITY);
            int diff = Math.Abs(featureScore - parts.MaterialPlusPst);
            Assert.IsTrue(diff <= 1, $"Out of sync on material + pst - expected: {parts.MaterialPlusPst}, actual: {featureScore}");

            // cached pawns
            featureScore = features.Compute(weights, HceWeights.ISOLATED_PAWN, HceWeights.KING_OUTSIDE_PP_SQUARE);
            diff = Math.Abs(featureScore - parts.CachedPawns);
            Assert.IsTrue(diff <= 1, $"Out of sync on cached pawns - expected: {parts.CachedPawns}, actual: {featureScore}");

            // mobility
            featureScore = features.Compute(weights, HceWeights.PIECE_MOBILITY, HceWeights.CENTER_CONTROL);
            diff = Math.Abs(featureScore - parts.Mobility);
            Assert.IsTrue(diff <= 1, $"Out of sync on mobility - expected: {parts.Mobility}, actual: {featureScore}");

            // king safety
            featureScore = features.Compute(weights, HceWeights.KING_ATTACK, HceWeights.ISOLATED_PAWN);
            diff = Math.Abs(featureScore - parts.KingSafety);
            Assert.IsTrue(diff <= 1, $"Out of sync on king safety - expected: {parts.KingSafety}, actual: {featureScore}");

            // pieces
            featureScore = features.Compute(weights, HceWeights.KNIGHT_OUTPOST, HceWeights.PAWN_PUSH_THREAT);
            diff = Math.Abs(featureScore - parts.PieceEval);
            Assert.IsTrue(diff <= 1, $"Out of sync on piece evaluation - expected: {parts.PieceEval}, actual: {featureScore}");

            // passed pawns
            featureScore = features.Compute(weights, HceWeights.KING_OUTSIDE_PP_SQUARE, HceWeights.KNIGHT_OUTPOST);
            diff = Math.Abs(featureScore - parts.PassedPawnEval);
            Assert.IsTrue(diff <= 1, $"Out of sync on passed pawn evaluation - expected: {parts.PassedPawnEval}, actual: {featureScore}");

            // threats
            featureScore = features.Compute(weights, HceWeights.PAWN_PUSH_THREAT, HceWeights.MAX_WEIGHTS);
            diff = Math.Abs(featureScore - parts.ThreatEval);
            Assert.IsTrue(diff <= 1, $"Out of sync on threat evaluation - expected: {parts.ThreatEval}, actual: {featureScore}");

            // miscellaneous
            featureScore = features.Compute(weights, HceWeights.CENTER_CONTROL, HceWeights.KING_ATTACK);
            diff = Math.Abs(featureScore - parts.MiscEval);
            Assert.IsTrue(diff <= 1, $"Out of sync on misc evaluation - expected: {parts.MiscEval}, actual: {featureScore}");
        }

        private int PrintSparseArray(EvalFeatures features, HceWeights wt)
        {
            SparseArray<short>[] sparse = features.Sparse;
            int phase = features.Phase;
            Color stm = features.SideToMove;
            IDictionary<int, short> coeff = features.Coefficients;
            Score total = Score.Zero;

            int count = 0;

            Console.WriteLine("  Key  : White | Black | Coeff |   Weight    = Score");
            Console.WriteLine("-----------------------------------------------------");
            IEnumerable<KeyValuePair<int, short>> sorted = coeff.OrderBy(kvp => kvp.Key);

            foreach (var kvp in sorted)
            {
                sparse[0].TryGetValue(kvp.Key, out short whiteValue);
                sparse[1].TryGetValue(kvp.Key, out short blackValue);
                Score s = (whiteValue - blackValue) * wt[kvp.Key];
                total += s;
                Console.WriteLine($"[{kvp.Key,5}]: {whiteValue,5} | {blackValue,5} | {kvp.Value,5} | {wt[kvp.Key],-12} = {s.NormalizeScore(phase)}");
                count++;
            }
            Console.WriteLine("-----------------------------------------------------");
            Console.WriteLine($"TOTAL:                                        = {total.NormalizeScore(phase)}");
            return count;
        }
    }
}